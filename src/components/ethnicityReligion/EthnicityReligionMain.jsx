import { Link } from "react-router-dom";
import FeaturedArticle from "./FeaturedArticle.jsx";
import SubArticleItem from "./SubArticleItem.jsx";
import EthnicityReligionThreeGrid from "./EthnicityReligionThreeGrid.jsx";
import EthnicityReligionVerticalList from "./EthnicityReligionVerticalList.jsx";
import EthnicityReligionSection from "./EthnicityReligionSection.jsx";
//style 
import "../../styles/ethnicityReligion/EthnicityReligionSection.css";
const EthnicityReligionMain = ({ featured, subList, gridList, verticalList }) => {
    return (
        <div className="politics-main">
            <div className="d-flex flex-wrap main-news">

                {/* LEFT: BÀI NỔI BẬT */}
                <div className="main-left">
                    <Link
                        to={`/article?url=${encodeURIComponent(featured.link)}&title=${encodeURIComponent(featured.title)}`}
                        className="text-decoration-none text-dark"
                    >
                        <FeaturedArticle data={featured} />
                    </Link>
                </div>

                {/* RIGHT: BÀI PHỤ */}
                <div className="main-right">
                    {subList.map(item => (
                        <SubArticleItem
                            key={item.id}
                            image={item.image}
                            title={item.title}
                            link={item.link}
                        />
                    ))}
                </div>
            </div>

            <div className="mt-4">
                <EthnicityReligionThreeGrid  data={gridList}/>
            </div>
            <div className="mt-4">
                <EthnicityReligionVerticalList data={verticalList}/>
            </div>
            <div className="news-row mt-4">
                <EthnicityReligionSection sections={gridList} />
                <EthnicityReligionSection sections={verticalList} />
            </div>
        </div>

    );
};

export default EthnicityReligionMain;
